...
% Nodes
\node[node] (content) at (0, 0) {Content};
...
% Edges
\draw[edge] (content) -- node[midway, above] {has model} (modelA);
\draw[edge] (content) -- node[midway, above] {has benchmark} (benchmark);
\draw[edge] (content) -- node[midway, above, sloped] {has code} (code);
